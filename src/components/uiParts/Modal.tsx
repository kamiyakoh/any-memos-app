...
export const Modal: FC<Props> = ({ selectedModal, onClose, children }) => {
  if (selectedModal === '') return null;

  const borderClass = modals.find((m) => m.name === selectedModal)?.borderClass ?? '';
  const handleBgClick = (event: React.MouseEvent<HTMLDivElement>): void => {
    if (selectedModal !== 'login' && onClose !== undefined && event.target === event.currentTarget) {
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={handleBgClick}>
      ...
      <div
        className={`relative text-black bg-white p-8 rounded-lg z-10 overflow-y-auto max-h-[90%] border-4 border-solid ${borderClass}`}
      >
        {selectedModal !== 'login' && (
