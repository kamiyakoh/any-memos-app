...
export const Modal: FC<Props> = ({ borderClass, isOpen, closeButton, closeOnBgClick, onClose, children }) => {
  if (!isOpen) return null;

  const handleBgClick = (event: React.MouseEvent<HTMLDivElement>): void => {
    if (closeOnBgClick === true && onClose !== undefined && event.target === event.currentTarget) {
      onClose();
    }
  };
